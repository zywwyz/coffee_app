...
当前个人 Debug 侧载范围已取得发布 GO。`72dcb28` 补齐饮用日期时间、持久草稿恢复、记录编辑/删除和 Room v2 迁移；`8e939f0` 进一步原子清理悬空编辑草稿并提供“放弃草稿并新建”。高风险复审与最终发布审计均 PASS，功能分支已快进合并到本地 `main`，待推送 GitHub 及真机验收。
...
1. 将本地 `main` 推送到 GitHub。
2. 有设备时补 connected test、安装和真机 OCR/SAF/恢复验收。

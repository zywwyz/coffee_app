...
以下图片由当前 Android 客户端的真实 Jetpack Compose 页面渲染生成，使用演示记录与实拍照片素材，展示奶油纸底、橄榄绿和照片便签样式。

| 咖啡日历 | 豆库 |
| --- | --- |
| <img src="docs/images/coffee-calendar.png" alt="咖啡日历：完整实拍图月历与最近一杯便签" width="300"> | <img src="docs/images/coffee-catalog.png" alt="豆库：品牌收藏卡与分类切换" width="300"> |
| 产品实拍卡 | 月度总结 |
| <img src="docs/images/coffee-products.png" alt="产品页：双列实拍卡与咖啡分类筛选" width="300"> | <img src="docs/images/coffee-insights.png" alt="总结页：饮用习惯便签与咖啡类型、品牌占比" width="300"> |
...
在“总结”选择月度或年度视图，可查看饮用天数、连续天数、消费与评分、与上期的杯数比较、咖啡类型与品牌占比、Top 3 偏好，以及最好／最差的历史记录卡。未填写价格或评分的记录仍会计入杯数，但不会被虚构为金额或评分；个人豆会归入“手冲”。
...
2. **恢复备份**：选择以前导出的 ZIP。恢复会替换当前全部数据，因此请先导出当前数据。当前备份格式为 v4，仍兼容旧版 v1、v2、v3；旧备份恢复时会为历史记录补全咖啡类型。

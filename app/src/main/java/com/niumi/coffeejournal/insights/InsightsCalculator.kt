...
object InsightsCalculator {
 ...
 fun yearly(year:Int,records:List<DrinkRecord>,today:String=LocalDate.now().toString()):YearlyInsights { val now=LocalDate.parse(today); val current=records.filter{date(it)?.year==year}; val n=if(year==now.year)now.monthValue else 12; val trend=(1..n).map{i->ComparisonPoint(i,current.count{date(it)?.monthValue==i},records.count{date(it)?.let { d -> d.year==year-1 && d.monthValue==i }==true}.takeIf{it>0})}; val legacy=(1..12).map { i -> TrendPoint("${i}月", null, 0, 0, null) }; val p=period(current); return YearlyInsights(year,p,0,legacy,emptyList(),emptyList(),emptyList(),null,trend=trend) }

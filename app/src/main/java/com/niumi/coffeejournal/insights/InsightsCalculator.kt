...
object InsightsCalculator {
 ...
 fun monthly(year:Int,month:Int,records:List<DrinkRecord>,previousMonthRecords:List<DrinkRecord>,today:String=currentLocalDate()):MonthlyInsights {
  ...
  val habit=p.habit.copy(cupDelta=if(year==1&&month==1)null else current.size-prior.count { date(it)?.day?.let { day -> day <= days } == true })
 ...
 private fun highlight(records:List<DrinkRecord>,best:Boolean):HighlightRecord? { val rated=records.filter{it.ratingHalfStars!=null}; val target=(if(best)rated.maxOfOrNull{it.ratingHalfStars!!} else rated.minOfOrNull{it.ratingHalfStars!!})?:return null; val groups=rated.filter{it.ratingHalfStars==target}.groupBy{if(it.sourceItemId.isNotBlank())it.sourceItemId else it.snapshot.brandName+"\u0000"+it.snapshot.itemName}; val chosen=groups.values.map(::latest).sortedWith(compareByDescending<DrinkRecord>{it.occurredAtEpochMillis}.thenBy{it.id}).first(); return HighlightRecord(chosen.id,chosen.snapshot.brandName,chosen.snapshot.itemName,target,chosen.snapshot.imageAssetId,chosen.snapshot.brandLogoAssetId,groups.size-1) }
